...
class PDF extends FPDF
{
   ...
   function Header()
   {
    include "../../conexion.php"; //llamamos a la conexion BD
      ...
      $this->Cell(80, 10, utf8_decode('MESA'), 1, 0, 'C', 1);
...
$consulta_Pedidos = $conexion->query(" select * from pedidos");

while ($datos_reporte = $consulta_Pedidos->fetch_object()) {
   $i = $i + 1;
   /* TABLA */
   $pdf->Cell(15, 10, utf8_decode($i), 1, 0, 'C', 0);
   $pdf->Cell(80, 10, utf8_decode($datos_reporte->num_mesa), 1, 0, 'C', 0);
   $pdf->Cell(30, 10, utf8_decode($datos_reporte->fecha), 1, 0, 'C', 0);
   $pdf->Cell(50, 10, utf8_decode($datos_reporte->total), 1, 0, 'C', 0);
}
